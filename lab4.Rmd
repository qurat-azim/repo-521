...
The code chunk option is evaluating the computation in this code chuck. I am using `eval=TRUE` as my option and this evaluates the results of the code placed in the chunk. If I remove this option, the reults of `5 * 15` will not be evaluated or printed.
...
```
I am using two options: the `eval=TRUE` option evaluates the last line of code. The `warning=TRUE` option helps print a warning that might be encountered during running the  code block.
